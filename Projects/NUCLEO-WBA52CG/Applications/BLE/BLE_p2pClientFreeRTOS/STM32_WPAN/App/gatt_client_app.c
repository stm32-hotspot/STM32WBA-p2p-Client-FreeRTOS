...
/* USER CODE BEGIN PFP */
/* FREERTOS_MARK */
static void GATTC_Discover_Entry(void* thread_input);
/* USER CODE END PFP */
...
void GATT_CLIENT_APP_Init(void)
{
  ...
  GATTC_Discover_Thread_Sem = osSemaphoreNew(1, 0, &GATTC_Discover_Thread_Sem_Attr);
  GATTC_Discover_Thread = osThreadNew(GATTC_Discover_Entry, NULL, &GATTC_Discover_Thread_Attr);
...
static void GATTC_Discover_Entry(void* thread_input)
{
  ...
  while(1)
  {
    osSemaphoreAcquire(GATTC_Discover_Thread_Sem, osWaitForever);
    client_discover_all();
  }
}

void P2Pclient_write_char(void)
{
  uint8_t index = 0;
